%% Seaweed ANN (MLP) 
%% Works best for cases {a1,a2,f1,f2} using [h1,h2] = [15,15], a=0.0001, L=1/30
clear all
close all
clc

% Load seaweed data 
Data = load("Negin_TimeTempDepencdency_TPC.txt");
...
NumNonInterp = length(Data_preInterp(round(entries_breakpoint)+1:end,1));


%%%%%%%%%%%%%%%Interpolation after data division%%%%%%%%%%%%%%%%%%%%%%%%%%%%
lw = 0.01;
x = 1:length(Data(1:round(entries_breakpoint)));
switch InpMethod
    case 'Mak'
        method = 'makima'
...
switch InpMethod
    case 'MultSpl'
        ...
        interpolated_Data = zeros(length(xq), size(Data(1:round(entries_breakpoint),:), 2));
        
        % Perform cubic spline interpolation for each variable
        for col = 1:size(Data(1:round(entries_breakpoint),:), 2)
            ...
            s = 1;
            spline_fn = csaps(x, Data(1:round(entries_breakpoint), col), s); 
            
            % Evaluate the spline at the query points
            interpolated_data(:, col) = fnval(spline_fn, xq);
            Data = vertcat(interpolated_data, Data(round(entries_breakpoint):end,:));
        end
        % Plot the results for visualization
        % figure;
        % for col = 1:size(Data, 2)
        %     subplot(2, 2, col);
        %     plot(x, Data(size(Data(1:round(entries_breakpoint), col))), 'o', 'MarkerFaceColor', 'r', 'DisplayName', 'Original Data');
        %     hold on;
        %     plot(xq, interpolated_Data(size(Data(1:round(entries_breakpoint)), col)), 'b-', 'DisplayName', 'Interpolated Data');
        %     title(['Variable ', num2str(col)]);
        %     legend;
        %     grid on;
        % end
    otherwise
        x = [1:round(entries_breakpoint)]';
        F = griddedInterpolant(x,Data(1:round(entries_breakpoint),:), method);
        qx = 1:ratio:length(Data(1:round(entries_breakpoint),1));        
        Vq = F(qx);
        Data_Int = Vq;        
        Data = vertcat(Data_Int,Data(round(entries_breakpoint):end,:));
end
...
switch Seaweed_Solvent_Combo
    case {'all','a12','f12'}
        ...
        % subplot(2,1,1)
        plot(cell2mat(Data_output((round(interpCut):end)')), 'kd',LineWidth=1);
        hold on;
        plot(cell2mat(predTrainData_outputs((round(interpCut):end)')), Color=colour, Marker='o', MarkerSize=8, LineWidth=lw);
        fontname('Times New Roman');
        ...
        % subplot(2,1,2)
        %     plot(error((round(interpCut):end)', 'kd')
    ...
    otherwise
       % Plot the actual vs. predicted values
        ...
        fontname('Times New Roman');
        % subplot(2,1,1)
        plot(Temp_axis((round(interpCut)):end),cell2mat(Data_output(round(interpCut):end)'), 'k:x',LineWidth=1);
        hold on;
        ...
        fontname('Times New Roman');
        legend('Actual', 'Predicted', 'Position',[0.717023807548341 0.697148487959284 0.17392857340404 0.0754761920202346]);
        xlabel('Temperature (\circ C)','FontSize',12);
        ylabel('Total Phenolic Content (mg/L)','FontSize',12);
        title(Title, subTitle2,'FontSize',10);
        % subplot(2,1,2)
...
switch Seaweed_Solvent_Combo
    case {'all','a12','f12'}
         % Plot the actual vs. predicted values
        figure('Name', 'model and error for all datapoints');
        fontname('Times New Roman');
        % subplot(2,1,1)
...
% %values extracted from plot: a1
% PredOutput_test = [0.789372 0.782928 0.781365 0.656667];
% expectedOutput_test = [0.79972 0.75021 0.72922 0.67209];
% errNARX = (-PredOutput_test + expectedOutput_test)./expectedOutput_test;

% % %values extracted from plot: f2
% PredOutput_test = [2.5592 2.50496 2.37796 2.21501];
% expectedOutput_test = [2.54759 2.4778 2.38731 2.33163];
% errNARX = (-PredOutput_test + expectedOutput_test)./expectedOutput_test;

% % %values extracted from plot: all
% PredOutput_test = [1.0511 1.20873 1.15812 1.05495 0.942546 0.850291 0.28326 0.405623 0.663343 0.835615 0.829088 0.805855 0.716226 0.651631];
% expectedOutput_test = [1.14043 1.14723 1.09277 1.02631 0.97419 0.1755 0.30837 0.55418 0.75007 0.78794 0.79972 0.75021 0.72922 0.67209];
% errNARX = (-PredOutput_test + expectedOutput_test)./expectedOutput_test;
